...
namespace SchoolApp
{
    public partial class ListaMerienda : Form
    {
        private string _nombre;
        private int _id;

        public ListaMerienda(string nombre, int id)
        {
            InitializeComponent();
            _nombre = nombre;
            _id = id;
        ...
        private void button1_Click(object sender, EventArgs e)
        {
            ...
            if (fila > 0)
            {
                ...
                string sql = $"SELECT * FROM comidas WHERE fecha = '{day}' AND curso_id = {_id}  AND tipo = 3";
                ...
                {
                    int lastId = 0;
                    sql = "INSERT INTO comidas output INSERTED.ID VALUES ('" + day + "'," + _id + ",3)";
                    ...
                    {
                        sql = $"INSERT INTO comidaest VALUES ({ids[i]},{lastId},'{estados[i]}',3)";
        ...
        private void ListaMerienda_Load(object sender, EventArgs e)
        {
            DataView dv;
            DateTime thisDay = DateTime.Today;
            var day = thisDay.ToString("d");
            ModelDataBase.executeQuery($"SELECT * FROM comidas WHERE fecha = '{day}' AND curso_id = " + _id + " AND tipo = 3");
            ModelDataBase.setDataAdpater("comidas");
            dv = ModelDataBase.getDataSet();
            DataTable dt = dv.ToTable();
            int dtrows = dt.Rows.Count;
            ...
            if (dtrows > 0)
            {
               ...
               "WHERE e.curso_id = " + _id + " AND e.curso_id = c.id AND e.id = a.estudiante_id AND a.comida_id = " + idComida + " AND a.tipo_id = 3");
